...
JNIEXPORT jlong JNICALL Java_suskun_nn_QuantizedDnn_getContext
        (JNIEnv *env,
         ...
         jint inputVectorCount,
         jint batchSize) {
    // create a new context and return it to the Java side for making subsequent calls on this context.
    dnn::CalculationContext *context = new dnn::CalculationContext(quantizedDnn, inputVectorCount, batchSize);
...
JNIEXPORT jfloatArray JNICALL Java_suskun_nn_QuantizedDnn_calculateForOutputs
        (JNIEnv *env, jobject obj, jlong handle, jint inputIndex, jintArray outputNodeIndexes) {
    dnn::CalculationContext *context = reinterpret_cast<dnn::CalculationContext *>(handle);
    jsize outputNodeCount = env->GetArrayLength(outputNodeIndexes);
    float *res = context->lazyOutputActivations(
            (int) inputIndex,
            env->GetIntArrayElements(outputNodeIndexes, 0),
            outputNodeCount);
